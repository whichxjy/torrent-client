...
const parseHave = (haveMessage) => {
  if (haveMessage.id !== HAVE) {
    throw new ParseError('Not a have message')
  } else if (haveMessage.payload.length !== 4) {
    throw new ParseError('The length of the payload is wrong')
  } else {
    return bytesToUint32(haveMessage.payload)
  }
}

// parse a "piece" message and get the index, begin and piece
const parsePiece = (pieceMessage) => {
  if (pieceMessage.id !== PIECE) {
    throw new ParseError('Not a piece message')
  } else if (pieceMessage.payload.length < 8) {
    throw new ParseError('Payload too short')
  } else {
    return {
      index: bytesToUint32(pieceMessage.payload.slice(0, 4)),
      begin: bytesToUint32(pieceMessage.payload.slice(4, 8)),
      piece: pieceMessage.payload.slice(8)
    }
  }
}
